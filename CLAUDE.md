...
(Flux / Z-Image) and audio playback from pre-rendered tracks. Runs alongside
the Python `sidequest-server` (per ADR-082; previously the Rust `sidequest-api`).
...
apply. Music is pre-rendered at build time (see ACE-Step project) and played
back from a file library — no runtime music inference lives here.
